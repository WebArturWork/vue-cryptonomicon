	...
	<div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
		...
		<div class="container">
			...
			<section>
				<div class="flex">
					<div class="max-w-xs">
						...
						<div class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
							<span @click="ticker = 'BTC'"
								class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer">
								BTC
							</span>
							<span @click="ticker = 'ETH'"
								class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer">
								ETH
							</span>
							<span @click="ticker = 'DOGE'"
								class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer">
								DOGE
							</span>
							<span @click="ticker = 'BCH'"
								class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer">
								BCH
							</span>
						</div>
				...
				<button @:click.enter="add" type="button"
			...
			<template v-if="tickers.length">
				...
				<dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
					<div v-for="t in tickers" :key="t" @click="select(t)" :class="{
						'border-4': sel === t
...
export default {
	...
	methods: {
		...
		add() {
			const currentTicksers = {
				name: this.ticker,
				price: "-"
			};

			this.tickers.push(currentTicksers);
			this.ticker = "";

			setInterval(async () => {

				const f = await fetch(
					`https://min-api.cryptocompare.com/data/price?fsym=${currentTicksers.name}&tsyms=USD&api_key=0850a160bd042cd383a368c6904c4b90222b29297556c9a8522881e235d36263`
				);

				const data = await f.json();
				this.tickers.find((t) => t.name === currentTicksers.name).price = data.USD > 1 ? data.USD.toFixed(2) : data.USD.toPrecision(2);

				if (this.sel.name === currentTicksers.name) {
